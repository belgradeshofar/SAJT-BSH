...
export default function ReligijaPage() {
  const [articles, setArticles] = useState<Article[]>([]);

  useEffect(() => {
    const fetchReligijaArticles = async () => {
      const { data, error } = await supabase
        .from("articles")
        .select("*")
        .eq("category", "RELIGIJA")
        .order("created_at", { ascending: false });

      if (error) {
        console.error("Greška prilikom učitavanja RELIGIJA:", error.message);
      } else {
        setArticles((data as Article[]) || []);
      }
    };

    fetchReligijaArticles();
  }, []);

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">RELIGIJA</h1>
      {articles.length === 0 ? (
        <p>Nema članaka u kategoriji RELIGIJA.</p>
      ) : (
