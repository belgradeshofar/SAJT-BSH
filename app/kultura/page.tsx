...
export default function KulturaPage() {
  ...
  useEffect(() => {
    const fetchKulturaArticles = async () => {
      ...
      if (error) {
        console.error("Greška prilikom učitavanja KULTURA:", error.message);
      } else {
        setArticles((data as Article[]) || []);
      }
    };

    fetchKulturaArticles();
  }, []);

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">KULTURA</h1>
      {articles.length === 0 ? (
        <p>Nema članaka u kategoriji KULTURA.</p>
      ) : (
